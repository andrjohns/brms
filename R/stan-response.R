...
copula_marginal <- function(family, link = "identity") {
  ...
  out <- glue(
    ...
    "    for (j in 1:J) {{\n",
    "      for (n in 1:N) {{\n",
    "        Ubound[n, j] = {family}_lcdf(Y[n, j] | {args_indexed});\n",
    "        Lbound[n, j] = (Y[n, j] > 0) ? {family}_lcdf(Y[n, j] - 1 | {args_indexed}) : negative_infinity();\n",
    "      }}\n",
    "    }}\n",
    "    rtn[2] = log_diff_exp_mat(Ubound, Lbound);\n",
    "    rtn[1] = std_normal_log_qf2(log_sum_exp_mat(Lbound, rtn[2] + log(URaw)));\n",
    "    return rtn;\n",
    "  }}\n"
  )
