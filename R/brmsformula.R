...
mvbrmsformula <- function(..., flist = NULL, rescor = NULL) {
  ...
  structure(
    nlist(forms, responses, rescor, copula),
...
#' @rdname brmsformula-helpers
#' @export
set_rescor <- function(rescor = TRUE, copula = NULL) {
  structure(as_one_logical(rescor), copula = copula, class = "setrescor")
}
...
validate_formula.mvbrmsformula <- function(
  ...
  if (is.null(formula$copula)) {
    formula$copula <- attr(formula$rescor, "copula")
  } else {
    formula$copula <- formula$copula
  }
